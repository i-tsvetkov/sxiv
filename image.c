...
bool img_load_gif(img_t *img, const fileinfo_t *file) {
  ...
  if (img->multi.cap == 0) {
    img->multi.cap = 8;
    img->multi.frames =
        (img_frame_t *)emalloc(sizeof(img_frame_t) * img->multi.cap);
  }
  ...
  do {
    ...
    if (rec == EXTENSION_RECORD_TYPE) {
      ...
      if (img->multi.cnt == img->multi.cap) {
        img->multi.cap *= 2;
        img->multi.frames = (img_frame_t *)erealloc(
            img->multi.frames, img->multi.cap * sizeof(img_frame_t));
      }
      img->multi.frames[img->multi.cnt].im = im;
      img->multi.frames[img->multi.cnt].delay =
          delay > 0 ? delay : DEF_GIF_DELAY;
...
bool img_pan(img_t *img, direction_t dir, int d) {
  ...
  if (d > 0) {
    ...
    x = img->win->w / (d < 0 ? 1 : ratio);
    y = img->win->h / (d < 0 ? 1 : ratio);
  }
