...
unsigned long win_alloc_color(win_t *win, const char *name) {
  ...
  if (XAllocNamedColor(win->env.dpy,
                       DefaultColormap(win->env.dpy, win->env.scr), name, &col,
                       &col) == 0) {
...
void win_check_wm_support(Display *dpy, Window root) {
  ...
  while (!found && length > 0) {
    if (XGetWindowProperty(dpy, root, atoms[ATOM__NET_SUPPORTED], offset,
                           length, False, XA_ATOM, &type, &format, &nitems,
...
void win_open(win_t *win) {
  ...
  win->xwin =
      XCreateWindow(e->dpy, RootWindow(e->dpy, e->scr), win->x, win->y, win->w,
                    win->h, 0, e->depth, InputOutput, e->vis, 0, NULL);
  if (win->xwin == None)
    error(EXIT_FAILURE, 0, "Error creating X window");

  XSelectInput(e->dpy, win->xwin, ButtonReleaseMask | ButtonPressMask |
